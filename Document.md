...
- `Ctrl+Enter`：次レコードへ（即時）
  - 次レコードがなければ新規作成して移動する。

### 5.3 本文セル内の改行・Enter挙動（確定）

本文セルでは、以下のルールで `Enter` を解釈する。

- `Enter`（通常）：基本は本文内改行
  - ただし、カーソル位置の「現在行」が空行の場合は、空行を残さず次レコードへ移動する  
