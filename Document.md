...
- 対象は本文セルのみとする（役名セル・概要モードには適用しない）。
- `Enter` / `Ctrl+Enter` / `Shift+Enter` など既存キー操作の意味は変更しない。
- 貼り付けテキストは、空行（空白のみ行を含む）区切りで段落候補を作る。
  - 段落内改行は保持する。
  - 空段落は無視する。
- 段落候補が2件以上の場合は確認ダイアログを表示する。
  - `はい`：分割して貼り付け（既定）
  - `いいえ`：通常貼り付け
- 分割して貼り付けを選んだ場合の適用:
  - 選択範囲を置換対象とし、現在本文を `before + 先頭段落` に更新する。
  - 2件目以降の段落は、直後に役名空の本文レコードとして順次挿入する。
  - 元本文の `after` は最終挿入レコード末尾に連結する。
- 段落候補が1件以下の場合は、通常貼り付け（`before + clipboard + after`）のみ行う。

### 5.6 役名セルへの貼り付け（確定）

- 対象は役名セルのみとする（本文セル・概要モードには適用しない）。
- 分割規則は本文セル貼り付けと同一とする。
  - 空行（空白のみ行を含む）区切りで段落候補を作る。
  - 段落内改行は保持する。
  - 空段落は無視する。
- `Enter` / `Ctrl+Enter` / `Shift+Enter` / `Tab` など既存キー操作の意味は変更しない。
- 段落候補が2件以上の場合は確認ダイアログを表示する。
  - `はい`：分割して役名を割り当て（既定）
  - `いいえ`：通常貼り付け
- 分割割り当て時の適用:
  - 貼り付け開始レコードから下方向に、レコード順で1件ずつ役名を全置換する。
  - 対象件数が不足する場合は確認ダイアログを表示する。
    - `はい`：不足分のレコードを末尾に追加し、件数分を割り当てる（既定）
    - `いいえ`：既存レコードに入る分のみ割り当てる（余りは破棄）
    - `キャンセル`：貼り付けを中止する
  - 割り当て対象に既存の役名が1件でも含まれる場合は、上書き確認（`はい`/`いいえ`、既定 `はい`）を表示する。
  - 既存本文（body）は変更しない。末尾追加時の新規レコード本文は空文字とする。
- 段落候補が1件以下の場合は、通常貼り付け（`before + clipboard + after`）のみ行う。
- Undo/Redo は貼り付け1回につき履歴1件とする。
...
- 改ページ溢れに対しては、ソフト側で自動分割は行わない（ユーザーが区切る方針）。
  - ただし本文セルの貼り付け時には、確認ダイアログ経由で分割貼り付けを選択できる。
