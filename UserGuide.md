# VerticalScenarioEditor (VSE) User Guide
...
### 3.4 本文セルへの貼り付け（分割貼り付け）

- 対象は本文セルのみです（役名セル・概要モードは対象外）。
- 貼り付け内容が空行区切りで複数段落になる場合、確認ダイアログが表示されます。
  - **はい（既定）**：段落ごとに本文レコードへ分割して貼り付けます。
  - **いいえ**：通常の貼り付けを行います。
- 分割貼り付けは編集位置に挿入されます。選択範囲がある場合はその範囲を置換します。
- `Enter` / `Ctrl+Enter` / `Shift+Enter` の既存挙動は変わりません。

### 3.5 役名セルへの貼り付け（分割割り当て）

- 対象は役名セルのみです。
- 貼り付け内容が空行区切りで複数段落になる場合、確認ダイアログが表示されます。
  - **はい（既定）**：段落を下方向のレコードへ順番に割り当てます（役名を全置換）。
  - **いいえ**：通常の貼り付けを行います。
- 分割割り当てを選んだとき、対象レコードが不足している場合は次を確認します。
  - **はい（既定）**：不足分のレコードを末尾に追加し、件数分を割り当てます。
  - **いいえ**：既存レコードに入る分だけ割り当てます（余りは破棄されます）。
  - **キャンセル**：貼り付けを中止します。
- 対象に既存の役名がある場合は上書き確認が表示されます（既定は **はい**）。
- 本文は変更されません。`Enter` / `Ctrl+Enter` / `Shift+Enter` / `Tab` の既存挙動も変わりません。
...
> なお、本文セル・役名セルの貼り付け時には、確認のうえ分割貼り付け（分割割り当て）を選択できます。
...
### Q. 本文に複数行を貼り付けると確認が出る
A. 貼り付け内容が空行区切りで複数段落になる場合、分割貼り付けの確認を表示します。`はい` で段落ごとにレコード化、`いいえ` で通常貼り付けです。

### Q. 役名に複数行を貼り付けると確認が出る
A. 役名セルでも、空行区切りで複数段落になる場合は確認を表示します。`はい` で下方向へ割り当て、件数不足時は「追加して件数分」か「可能分のみ」かを選べます。
